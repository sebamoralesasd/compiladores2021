...
addsupTy :: MonadFD4 m => Name -> STy -> m ()
addsupTy n (STypeSinonym name) = 
  do
    maybeTy <- lookupSTy name
    case maybeTy of
                  Nothing -> failPosFD4 NoPos $ "Error"
                  Just ty -> modify (\s -> s {supTyEnv = (n, ty) : supTyEnv s})
addsupTy n supTy = 
  do 
    ty <- desugarTy supTy
    modify (\s -> s {supTyEnv = (n, ty) : supTyEnv s})
